...
# Add rendering framerate to config
...
def calculateContextCost(prev_shot,shot):
    ...
    # if(shot==prev_shot):
    #     return 0
    if (shot_n<prev_shot_n):
        return 0.01*cost_norm_factor
    else:
...
def loadConfig(config_file):
    ...
    global DEBUG_MODE, output_video_name, transition_factor, video_frames, \
        rythm_factor, low_overlap_factor, high_overlap_factor, audio, \
        low_overlap_percentage, mid_overlap_percentage, mid_overlap_factor, \
        establishing_shot_time, videoName, video, shots, actors, width, \
        gaze, gazeXOffset, gazeYOffset, normFactorX, normFactorY, shot_keys, \
        height, min_shot_duration, low_transition_factor, \
        high_transition_factor

    DEBUG_MODE = config_reader.getint('exec', 'DEBUG_MODE', fallback=0)
    output_video_name = config_reader.get('exec', 'output_video_name',
                                          fallback='output.mp4')

    high_transition_factor = config_reader.getfloat('parameters',
                                                    'high_transition_factor',
                                                    fallback=1)
    low_transition_factor = config_reader.getfloat('parameters',
                                                   'low_transition_factor',
                                                   fallback=0.01)
    rythm_factor = config_reader.getfloat('parameters',
                                          'rythm_factor',
                                          fallback=2)
    min_shot_duration = config_reader.getint('parameters',
                                             'min_shot_duration',
                                             fallback=60)
    ...
    high_overlap_factor = config_reader.getfloat('parameters',
                                                 'high_overlap_factor',
                                                 fallback=1000)
    establishing_shot_time = config_reader.getint('parameters',
...
def loadShotTracks():
    ...
    """
    print('Loading Shot Tracks ...')
    shot_tracks = {}
    for shot in shot_keys:
        DEBUG_MODE and print(Fore.WHITE+'Loading shot '+shot +' from : ' + shots[shot])
...
def analyticRythmFunction(time):
    ...
    low_rythm_factor = cost_norm_factor*low_rythm_factor
    high_rythm_factor = cost_norm_factor*high_rythm_factor
    max_shot_duration = 144
    rythm_cost = -high_rythm_factor/(1+np.exp(time-max_shot_duration)
                            ) + low_rythm_factor+high_rythm_factor
...
def analyticTransitionFunction(time):
    ...
    #     transition_cost = high_transition_factor * cost_norm_factor
    # else:
    #     transition_cost = low_transition_factor * cost_norm_factor

    #new transition function starts here (To be named min shot duration function)
    high_factor = high_transition_factor * cost_norm_factor
    low_factor = low_transition_factor * cost_norm_factor
    transition_cost = -high_factor/(1+np.exp(-time+min_shot_duration)) + low_factor+high_factor
...
def initialiseGlobalMatrices():
    ...
    shot_composition_cost = {
        'MS': lambda width_ratio: 0.01*cost_norm_factor
        if width_ratio > 0.4 else 0,
        'FS': lambda width_ratio: 0.01*cost_norm_factor
        if width_ratio < 0.4 else 0
    }
...
def analyticShotCompositionFunction(width_ratio, shot_composition):
    ...
    #                  if width_ratio < 0.3 else 0
    # }
    return shot_composition_cost[shot_composition](width_ratio)
    # if(width_ratio<0.25):
...
def renderVideoFromCroppedWindow(input_video,
    ...
    if(audio[videoName] != 'NA'):
        output_video_name = addAudioToVideo('temp.mp4',
                                            audio[videoName],
                                            output_video_name)
    else:
        shell_command = 'rename temp.mp4 '+output_video_name
...
def playWithGaze(videoName, rectangle, gaze_tracks, index=0):
    ...
    while index < no_of_frames-1:
        ...
        cropped_frame = cv2.resize(orig_frame, (int(1.7779*720), 720))
        frame_text = 'Frame : '+str(index)
        shot_text = 'Shot : '+final_track[index]
        cv2.putText(cropped_frame, frame_text,
...
if __name__ == '__main__':
    ...
    if DEBUG_MODE:
        ...
        print('Low Transition Factor : '+str(low_transition_factor))
        print('High Transition Factor : '+str(high_transition_factor))
        ...
        print('Rythm Factor : '+str(rythm_factor))
    ...
    for present_frame in range(establishing_shot_time, frames_in_range):
        ...
        DEBUG_MODE and print(Fore.RED+"Frame : ", present_frame)
        addDynamicCosts(present_frame)
        DEBUG_MODE and print(Fore.YELLOW+"Unary Cost : " +
                             str(display_cost(present_frame, cost)))
        DEBUG_MODE and print(Fore.MAGENTA+"Dp Cost : " +
                             str(display_cost(present_frame, dp_cost)))
        DEBUG_MODE and print(Fore.CYAN+"back_track : " +
                             str(display_cost(present_frame, back_track)))
        DEBUG_MODE and print(Fore.WHITE+"shot_duration : " +
    ...
    DEBUG_MODE and print([str(idx)+':'+shot
    ...
    if DEBUG_MODE:
        ...
        y = [analyticTransitionFunction(t) for t in x]
        ...
        ax = plt.subplot(323)
        # overlap cost
        ax.set_title("Overlap Cost")
        x = np.linspace(0, 1, 200)
        y = [analyticOverlapFunction(t) for t in x]
        cnf = [cost_norm_factor for t in x]
        plt.plot(x, y, label='Overlap cost')
        plt.plot(x, cnf, 'r-', label='Cost norm Factor')
        plt.legend()
